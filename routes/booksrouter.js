...
router.post("/", async (req, res) => {
  try {
    const { tittle, author, isbn, published_year, price, stock } = req.body;

    if (!tittle || !author || !isbn || !published_year || !price || !stock) {
      return res.status(400).json({ message: "Semua kolom wajib diisi" });
    }

    const bookId = await Books.addBook({ tittle, author, isbn, published_year, price, stock });
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const updated = await Books.updateBook(req.params.id, { tittle, author, isbn, published_year, price, stock });
    if (!updated) {
      return res.status(404).json({ message: "Buku tidak ditemukan" });
    }
